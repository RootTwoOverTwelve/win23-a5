...
# Test 05: Ensure correct rounding with division...
if [[ $($CALCULATOR 10 / 3) -ne 3 ]]; then  # If the output of the program is not 3...
  echo 'ERROR! A valid run of the calculator (10 / 3) failed to produce 3 as an output!'
  exit 1
fi

# Test 06: Ensure simple subtraction has correct output...
if [[ $($CALCULATOR 21 - 7) -ne 14 ]]; then  # If the output of the program is not 14...
  echo 'ERROR! A valid run of the calculator (21 - 7) failed to produce 14 as an output!'
  exit 1
fi